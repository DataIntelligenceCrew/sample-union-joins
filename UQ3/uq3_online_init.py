...
def gen_os(js, hs, alpha, e_j, pri_keys):
    ...
    for subset in ans:
        ...
        max_accu_olp, max_pr = wander_gen_o(new_js, new_hs, alpha, e_j[index], new_pri_keys)
...
def uq3_online_init_nb(js, hs, alpha, pri_keys):
    Js = []
    for join_index in range(len(js)):
        Js.append(wander_e_size(alpha, js[join_index], hs[join_index], pri_keys[join_index]))
